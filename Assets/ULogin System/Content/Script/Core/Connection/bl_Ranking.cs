...
public class bl_Ranking : MonoBehaviour {
    ...
    IEnumerator GetTop()
    {
        ...
        WWW www = new WWW(bl_LoginDataBase.Instance.GetUrl(bl_LoginDataBase.URLType.Ranking));
        ...
        else
        {
            ...
            foreach (string u in splituser)
            {
                ...
                if (u != string.Empty && u != null)
                {
                    ...
                    if (splitinfo[0] == bl_LoginDataBase.Instance.Admin || st == 2)
                    {
                        user.isAdmin();
                    }
                    if (st == 1)
